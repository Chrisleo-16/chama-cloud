...
export default function Overview() {
  const { data: groups, isLoading: gL, isError: gE, error: gErr } = useQuery({ queryKey: ["groups"], queryFn: groupsApi.list });
  const { data: contributions, isLoading: cL, isError: cE, error: cErr } = useQuery({ queryKey: ["contributions"], queryFn: contributionsApi.list });
  const { data: profile, isLoading: pL, isError: pE, error: pErr } = useQuery({ queryKey: ["profile"], queryFn: userApi.getProfile });
  ...
  if (gL || cL || pL) return (
    <div className="flex items-center justify-center h-80">
      <div className="text-center space-y-4">
        ...
        <p className="text-xs text-[var(--fg-muted)] mt-2">Groups: {gL ? 'Loading...' : 'Loaded'} | Contributions: {cL ? 'Loading...' : 'Loaded'} | Profile: {pL ? 'Loading...' : 'Loaded'}</p>
      </div>
    </div>
  );
  ...
  const myGroups = (groups || []).filter((g) => {
    ...
    // Merchant sees groups where they have made contributions
    if (userRole === "MERCHANT") {
      return myContributions.some((c) => c.group === g.id);
    }
  ...
  const walletBalance = completedContributions; // Only count completed contributions
  ...
  const referralLink = referralToken
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}/register?ref=${encodeURIComponent(referralToken)}`
    : "";
  ...
  const activities = myContributions
    ...
    .map((c) => ({
      id: c.id,
      user: "You", // Always show as "You" since these are user's own contributions
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {!myGroups?.length && (
                <div className="cc-card col-span-2 flex flex-col items-center py-14 gap-3 text-center">
                  ...
                  <button onClick={() => navigate('/dashboard/groups')} className="cc-btn-primary text-sm"><PlusCircle className="w-4 h-4" />Browse Groups</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="cc-card cc-fade cc-d3">
              ...
              <div className="space-y-2">
                {[
                  { label: 'New Chama',     icon: PlusCircle, color: 'brand', path: '/dashboard/new-chama' },
                  { label: 'Invite Member', icon: Send,        color: 'brand', path: '/dashboard/invite' },
                  { label: 'View Reports',  icon: BarChart3,  color: 'accent', path: '/dashboard/reports' },
                ].map(a => (
                  <button key={a.label} onClick={() => navigate(a.path)} className="w-full flex items-center gap-3 p-3 rounded-xl border border-[var(--border)] hover:border-[var(--brand-border)] hover:bg-[var(--brand-tint)] transition-smooth text-left group">
