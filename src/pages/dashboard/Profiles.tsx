...
import { Loader2, User, Phone, Briefcase, Shield, LogOut, Building, Leaf } from "lucide-react";
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-16">
      ...
      <div className="cc-card flex items-center gap-5 cc-fade cc-d1">
        ...
        <div>
          <h2 className="font-bold text-[var(--fg)] text-xl">{profile?.first_name} {profile?.last_name}</h2>
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-4 px-5 py-4 border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-alt)] transition-smooth">
            ...
            <div>
              ...
              <p className={`font-medium text-[var(--fg)] ${(row as { mono?: boolean }).mono ? "font-mono" : ""}`}>{row.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Sign out */}
      <button onClick={logout} className="cc-btn-ghost w-full gap-2 text-[var(--danger)] hover:!bg-[var(--danger-bg)] cc-fade cc-d3">
