...
export default function Groups() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<ChamaGroup> }) =>
  ...
  const totalFunded =
    groups?.filter((g) => g.is_fully_funded === "True").length ?? 0;
  const totalActive = groups?.filter((g) => g.is_active).length ?? 0;
  const totalCapital =
    groups?.reduce((s, g) => s + parseFloat(g.current_amount || "0"), 0) ?? 0;
    ...
    <div className="space-y-8 max-w-[1440px] mx-auto pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 cc-fade">
        ...
        <Dialog
          ...
          onOpenChange={(v) => {
            setDialogOpen(v);
            if (!v) setEditGroup(undefined);
          }}
        >
          <DialogTrigger asChild>
            <button className="cc-btn-primary flex-shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered?.map((g, idx) => {
            ...
            return (
              <div
                key={g.id}
                className="cc-card hover-lift cc-fade cursor-pointer group"
              ...
              >
                ...
                {/* Description */}
                {g.description && (
                  <p className="text-xs text-[var(--fg-muted)] line-clamp-2 mb-4">
                    {g.description}
                  </p>
                )}
                ...
                <div className="flex gap-2 mb-4">
                  <span
                    className={`cc-badge ${g.is_active ? "cc-badge-brand" : "cc-badge-neutral"}`}
                  >
